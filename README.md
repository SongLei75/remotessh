...
- `config/gcpp/baton.ssh_config`: OCI E1 first-hop configuration used by the jump-server simulator path.
- `scripts/build-baton-wolfssh.sh`: builds the E1 test-only wolfSSH binary.
- `scripts/test-direct.sh`: validates local company wolfSSH through a GCP IAP TCP tunnel to the GCP PKIX-SSHD simulator.
- `scripts/test-baton.sh`: validates the OCI jump-server path to the same simulator.

Current session paths:

```text
Direct: BoardSession -> local company wolfssh -> IAP TCP tunnel -> GCP PKIX-SSHD
Jump:   BoardSession -> ssh2 -> OCI E1 -> remote company wolfssh -> GCP PKIX-SSHD
```
